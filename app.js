...
var dataMenuOpen = false;
        ...
        // Toggle Scope / Signals / Display control groups independently of
        // the main Filters toggle. The button lives at the right end of the
        // search-ticker row so it's always reachable even when filters are open.
        window._screenerControlsVisible = true;
        function toggleScreenerControls() {
          const groups = document.getElementById('screenerFilterChips');
          if (!groups) return;
          window._screenerControlsVisible = !window._screenerControlsVisible;
          const show = window._screenerControlsVisible;
          // Respect the outer Filters toggle: only show if Filters itself is also visible
          const filtersOpen = groups.style.display !== 'none';
          if (show) {
            // Reveal — but only if the parent Filters wrap is open; if not, open it too
            if (!filtersOpen) {
              groups.style.display = '';
              const toggleChips = document.getElementById('screenerToggleChips');
              if (toggleChips) toggleChips.style.display = '';
              document.getElementById('filtersToggleLabel').textContent = 'Hide Filters';
            }
          } else {
          ...
          if (icon) {
            // Swap to an eye-off icon when hiding, eye-on when showing
            icon.innerHTML = show
